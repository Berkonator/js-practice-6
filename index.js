...
function hasProperty(obj, property) {
  let result = "";
  if(property in obj){
    result += "Property " + property + " exists";
  } else{
    result += "Property " + property + " does not exists";
  }
...
function destructureObject(obj) {
  let a = `brand: ${laptop.brand}`;
  let b = `Model: ${laptop.model}`;
  return {a, b}
...
console.log(changeRole(userList, "editor"));
// Виведе
...
function printProductDetails(obj) {
  let a = `${product.productName} ${product.price} ${product.manufacturer.companyName} ${product.manufacturer.country}`
  return a
...
console.log(printProductDetails(product)); // Виведе Smartphone 699 Tech Corp USA
